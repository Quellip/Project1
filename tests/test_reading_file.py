...
def test_get_reading_csv_not_file() -> None:
    result = get_reading_csv("test_transactions1.csv")
    assert result == []


@patch("src.reading_file.pd.read_excel")
def test_get_reading_exel(mock_get: Any) -> None:
...
@patch("src.reading_file.pd.read_excel")
def test_get_reading_exel_empty(mock_get: Any) -> None:
